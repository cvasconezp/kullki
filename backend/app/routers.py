...
def precancelar_credito(credito_id: int, db: Session = Depends(get_db),
                        user: models.Usuario = Depends(require_roles("tesorero", "superadmin"))):
    """Liquidación anticipada: cobra solo el capital pendiente, condona intereses futuros."""
    ...
    for cuota in pendientes:
        ...
        cuota.registrado_por = user.id
    credito.estado = "pagado"
    log_audit(db, user, "precancelar", "credito", credito.id,
              f"Liquidación anticipada: ${capital_restante:.2f} de capital — crédito #{credito.id} de {credito.socio.nombres}",
              caja_id=credito.caja_id, afecta_socio_id=credito.socio_id)
    db.commit()
    db.refresh(credito)
    return _credito_out(credito, detalle=True)
